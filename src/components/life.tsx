...
const LifeGame: FC = () => {
    const lifeMatrix = useRef<LifeMatrix>(new LifeMatrix(
            getRandomMatrix(0, 1, lifeGameConfig.dimension, lifeGameConfig.dimension)));
    const [numbers, setNumbers] = useState<number[][]>(lifeMatrix.current.matrix);

    function tic() { setNumbers(lifeMatrix.current.nextStep());}
    useEffect(() => {
